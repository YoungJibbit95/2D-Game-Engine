...
UI has a functional debug/HUD baseline, main menu, loading flow, pause menu, editable settings menu with tabs, mouse row selection, key capture, conflict warnings, reset confirmation, shared minimalist theming, opacity controls, reduced-motion controls, and slide/fade animation primitives. It still needs a proper UI toolkit: focus, typed text fields, tooltips, visible drag/drop widgets powered by the core cursor item model, inventory windows, equipment widgets, crafting screens that consume `CraftingQueryResult`, world-select/create-world flows, and debug panels.
