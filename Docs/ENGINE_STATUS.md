...
Rendering needs the next production pipeline: texture atlases, chunk render caches, autotile source rects, render targets, proper shader passes, entity animations, particle rendering, and draw metrics. Placeholder tile and light rendering is now negative-X aware for horizontally infinite worlds, and real tile PNGs can be resolved through `ClientTextureRegistry` once generated.
