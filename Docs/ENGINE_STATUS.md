...
UI has a functional debug/HUD baseline, main menu, loading flow, pause menu, editable settings menu with tabs, mouse row selection, key capture, conflict warnings, reset confirmation, shared minimalist theming, opacity controls, reduced-motion controls, slide/fade animation primitives, and a new core UI toolkit for layout, focus traversal, hit-testing, modal layers, and tooltips. It still needs client renderer widgets and screens for typed text fields, visible drag/drop widgets powered by the core cursor item model, inventory windows, equipment widgets, crafting screens that consume `CraftingQueryResult`, world-select/create-world flows, and debug panels.
