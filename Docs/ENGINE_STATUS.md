...
The project has a solid MonoGame/.NET 8 shell with a clean `Game.Core` separation. Core already covers chunked world storage, finite and horizontally infinite world bounds, chunk metadata, tile coordinate helpers, deterministic world generation, streamed chunk save/load, content registries, sprite asset manifests, mod merge reporting, save/load foundations, inventory, cursor item interactions, data-driven item actions, crafting query states, nearby crafting station discovery, loot, combat health, projectiles, entity definitions, enemy runtime entities, slime AI, spawning rules, command console support, settings, underground-aware lighting, time, spatial queries, and a core runtime simulation tick with world-simulation scheduling.

The client now has a main menu, a loading state, a keyboard/mouse-driven settings menu, an in-game pause menu, a playable state, camera follow, debug overlays, hotbar UI, visible tile/liquid rendering, and mouse-based selected item use for mining, building, and melee. Resolution, fullscreen, and VSync settings apply live through the client shell. Rendering is still placeholder-driven, but there is a first pass at render layers, shader registry scaffolding, and post-processing settings.
...
Rendering needs a real asset pipeline: texture atlases, chunk render caches, autotile source rects, render targets, proper shader passes, entity animations, particle rendering, and draw metrics. Placeholder tile and light rendering is now negative-X aware for horizontally infinite worlds.
