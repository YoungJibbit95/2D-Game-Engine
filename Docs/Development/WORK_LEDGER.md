...
Last updated: 2026-07-19 (budgeted physics/movement, active liquids, swept combat queries and finite-world generation workspace)
...
- Replaced the lossy viewport-height panorama fit with an authored-density contract. Surface composites retain their exact 4:1 aspect ratio and native `1536x384` extent at 720p, 1080p and 1440p, including 3440/5120 ultrawide, and use bounded whole-pixel scaling only on 2160p-class output. Camera zoom, jumping, terrain depth and large negative X positions cannot change image density; seamless repeats use no mirror, alternation, jitter, overlap or fractional filtering. Palette-matched top fill and terrain-hidden bottom fill cover outside the panorama without stretching its image area. The focused Parallax scope passes 76/76 in Debug and Release at 0 B. Final 1920x1080 Forest and 3440x1440 Amber Release captures pass with 215 resources, 1,168 frames and 0 invalid; background Draw averages 0.1105/0.1068 ms with 0 B.
- Moved sideview/topdown controllers into `Game.Core.Movement`; the sideview controller now applies intent only while `PhysicsWorld` owns player gravity, force integration and tile collision. Generic dynamic/kinematic/static bodies, materials, layer/mask filtering, detailed contacts, swept/substepped tile collision and deterministic heap-sort sweep-and-prune use caller-owned storage. Capacity overflow fails fast instead of slowing simulation time. Final isolated Release fixtures measure 1,000 settled bodies at 0.587 ms/step and reverse-ordered overlap queries at 0.244 ms/query, both at 0 B.
...
- Added opacity-only authored-horizon feathering and corrected premultiplied parallax/reflection tints. The panorama keeps native pixel density and repeat mapping while the previous hard top-fill seam is removed; final 1080p Background Draw averages 0.056 ms at 0 B.
- Replaced the active opaque/feathered panorama path with twelve independent binary-alpha V6 Far/Mid/Near planes for Forest, Amber Grove, Twilight Marsh and Crystal Depths. Far silhouettes key authored sky organically, Mid/Near dissolve into the procedural atmosphere, fixed density prevents jump zoom and distinct 0.08/0.18/0.32 parallax ratios create Terraria-like depth without PNG-edge cutoff. The normal Far repeat is back to one Draw, and one redundant background SpriteBatch End/Begin boundary is removed.
- Completed Tree V2 with twelve deterministic root/branch/crown silhouettes and three 16-mask oak/pine/birch foliage atlases; negative canopy openings and denser overlapping clusters replace rectangular crowns and hollow leaf loops.
...
| Strict V6 visual asset audits | background depth planes 12/12, Wave 06 character layers 5/5 and loose foliage atlases 3/3 pass with 0 hard issues |
...
| Final V6 visual-polish smoke | 2560x1440 Forest passes at frame 360 with 235 resources, 1,308 frames and 0 invalid; Background Draw averages 0.063 ms at 0 B, total CPU Draw averages 0.702 ms and the cutoff-free Far/Mid/Near composition was visually inspected |
...
Continue the renderer/UI presentation exit by capturing Forest, Amber Grove, Twilight Marsh and Crystal Depths at 720p/1440p during traversal, jumping and mining; tune Mid/Near opacity only from those comparisons. Then map compiled render-graph alias slots onto a descriptor-keyed transient target pool, add bounded active-biome/cross-category atlas residency and expose optional backend GPU timestamps. Migrate remaining inventory/crafting surfaces to the status-dock material language and replace the highest-impact legacy enemy/item sprites under the same brief/provenance/audit contract. Retain full Debug/Release, 0 B background/status planners and renderer p95/p99 gates.
...
- Water Presentation: prepared surface/depth/shore transitions plus shared reflection surfaces without new Draw work.
