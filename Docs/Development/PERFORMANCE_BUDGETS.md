...
Last updated: 2026-07-19 after the V6 background, Wave 06, Tree V2, liquid-presentation and central entity-physics pass
...
| Procedural background Draw | <= 1.25 ms average | V6 0.089-0.113 ms at 1080p and 0.134 ms at 3440x1440; 0 B | `measured` | 120/90 measured frames after equal warmups; exact 4:1 planes use uniform X/Y viewport-tiered authored-distance scales that are camera/terrain invariant, point sampled and unmirrored; top/bottom coverage is separate |
...
- Texture correctness is now measurable through 235 resources and 1,308 frames. The 34.74 MB decoded all-content preload is acceptable for the current sample but is not a scalable policy.
...
Epic 2 Milestone 5 now has compact streaming windows, columnar immutable snapshots, deterministic spawn warm-start, active-cell liquid plus prepared surface presentation, intent-separated buffered movement, centralized enemy/item physics with body impulses, swept/time-ordered combat, bounded nearest homing and chunk-local finite generation, with a green 1381-test Debug/Release tree and V6 1080p/1440p traces. The next measurement package covers one-way/sloped tile shapes, continuous fast body pairs, water-heavy visual fixtures, large regional generation, liquid pressure/equalization and dense adversarial combat. GPU time stays unknown until a backend-specific timestamp-query contract exists.
